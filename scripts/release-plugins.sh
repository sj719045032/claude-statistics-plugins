...
# Pre-flight: each plugin's `static let manifest = PluginManifest(...)`
# now resolves through the SDK helper `PluginManifest(bundle:)`, which
# reads the bundle's Info.plist `CSPluginManifest` dict at runtime.
# That makes the plist the single source of truth for id / version /
# minHostAPIVersion / etc. — there's no Swift-side copy to drift from
# anymore, so the old Swift↔plist parity check has been removed. The
# stamping below (Step 3) reads each plist directly and writes the
# same values into index.json so the marketplace metadata matches the
# bundle.
